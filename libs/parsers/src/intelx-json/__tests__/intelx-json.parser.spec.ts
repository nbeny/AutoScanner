import { IntelxJsonParser } from '../intelx-json.parser';
...
describe('IntelxJsonParser', () => {
  const parser = new IntelxJsonParser();
  ...
  it('maps probe exposures[] to NormalizedBreachExposure using the explicit fields', async () => {
    ...
    const paste = out.breachExposures.find((b) => b.breachName === 'exposed-paste');
    expect(paste).toMatchObject({ passwordExposed: false, severity: 'LOW' });
    expect(paste?.breachDate).toBeUndefined();
  });
