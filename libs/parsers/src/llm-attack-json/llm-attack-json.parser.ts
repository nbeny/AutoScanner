...
@Injectable()
export class LlmAttackJsonParser implements Parser {
  readonly name = 'llm-attack-json';
  ...
  async parse(input: Buffer | string, ctx: ParserContext): Promise<NormalizedOutput> {
    ...
    let report: { endpoint?: string; findings?: unknown[] };
    try {
    ...
    const location = report.endpoint ?? ctx.target;
