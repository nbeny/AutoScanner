...
@Injectable()
export class GitDumperJsonParser implements Parser {
  readonly name = 'git-dumper-json';
  ...
  async parse(input: Buffer | string, ctx: ParserContext): Promise<NormalizedOutput> {
    ...
    let report: { gitUrl?: string; findings?: unknown[] };
    try {
    ...
    const location = report.gitUrl ?? ctx.target;
