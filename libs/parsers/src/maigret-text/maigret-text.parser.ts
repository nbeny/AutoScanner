...
/**
 * Parser for maigret `--print-found` stdout. Lines `## SEED <value>` (emitted by
 * the scanner's build script) set the current seed; `[+] <Service>: <url>` lines
 * become USERNAME identities. Tolerant of blank / unrecognised lines.
 */
@Injectable()
export class MaigretTextParser implements Parser {
  readonly name = 'maigret-text';
