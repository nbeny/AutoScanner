 ...
 * Phase (breach/auth modern scanners) — appends leakix + intelx breach-intel
 * lookups against the engagement target. Both require an operator-managed
 * free-tier API key (LEAKIX / INTELX); the scan-worker resolves credentials
 * at dispatch time.
 */
export const OsintMetaDeep: TemplateDefinition = {
  ...
  description:
    ...
    '→ fofa → urlfinder → optional gitleaks → emailrep → leakix + intelx breach-intel lookups. ' +
    'Zero-contact with the target; every step queries third-party data sources only. Reuses ' +
    'operator API credentials when present.',
  steps: [
    ...
    { scannerName: 'leakix', inputs: {}, target: { kind: 'context', path: 'target' } },
    { scannerName: 'intelx', inputs: {}, target: { kind: 'context', path: 'target' } },
  ],
};
