// Phishing-exposure correlation used in generated reports. This mirrors the
// frontend helper in apps/frontend/src/features/osint/phishing-exposure.ts —
// keep the two in sync (both are covered by unit tests). It is intentionally
// framework-agnostic (no Node/DOM deps) so it can run inside the report-worker.
