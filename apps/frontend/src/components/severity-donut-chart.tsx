...
/**
 * Pure presentational donut for a severity breakdown. Shared by the
 * engagement Overview (`SeverityDonut`) and the global dashboard so both
 * render an identical chart from a plain `SeverityCounts` value.
 */
export function SeverityDonutChart({ counts }: { counts: SeverityCounts }) {
  ...
  let angle = -Math.PI / 2;
  const arcs = ORDER.filter((k) => counts[k] > 0).map((k) => {
      ...
      <ul className="text-xs space-y-1">
        {ORDER.map((k) => (
          <li key={k} className="flex items-center gap-2">
            ...
            <span className="font-mono text-slate-100">{counts[k]}</span>
          </li>
        ))}
      </ul>
    </div>
