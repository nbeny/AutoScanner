...
function deps() {
  ...
  return { prisma, queue, provisioner };
}

describe('ChainLauncher', () => {
  it('rejects an unknown chain', async () => {
    const { prisma, queue, provisioner } = deps();
    const svc = new ChainLauncher(prisma, queue, provisioner, registry);
  ...
  it('creates a CHAIN AiRun and enqueues it', async () => {
    ...
    expect((queue as { add: jest.Mock }).add).toHaveBeenCalledWith('airun', {
      aiRunId: 'r1',
      engagementId: 'e1',
    });
  });
