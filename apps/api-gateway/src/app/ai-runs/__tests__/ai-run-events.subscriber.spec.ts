...
import {
  ...
  aiRunEventsChannel,
  type AiRunEventMessage,
} from '../ai-run-events.subscriber';
...
describe('aiRunEventsChannel', () => {
  it('produces a stable, scoped channel name', () => {
    expect(aiRunEventsChannel('r1')).toBe('airun:events:r1');
  });
});

describe('AiRunEventsSubscriber', () => {
  it('yields a parsed event after a message is emitted on the run channel', async () => {
    const redis = new FakeRedis();
    const subscriber = new AiRunEventsSubscriber(redis as never);

    const iter = subscriber.subscribe('r1');
    // let the async subscribe() register on the channel
    await new Promise((r) => setImmediate(r));
    expect(redis.subscribe).toHaveBeenCalledWith('airun:events:r1');

    redis.emit('message', 'airun:events:r1', JSON.stringify({ type: 'status', status: 'RUNNING' }));

    const [event] = await collect(iter, 1);
    expect(event).toEqual<AiRunEventMessage>({ type: 'status', status: 'RUNNING' });
  ...
  it('does not throw and does not yield on malformed JSON', async () => {
    ...
    expect(() => redis.emit('message', 'airun:events:r3', '{not json')).not.toThrow();
    // a valid message after the bad one is still delivered
    redis.emit('message', 'airun:events:r3', JSON.stringify({ type: 'audit-ready' }));
  ...
  it('unsubscribes when the last consumer leaves via return()', async () => {
    ...
    expect(redis.unsubscribe).toHaveBeenCalledWith('airun:events:r4');
